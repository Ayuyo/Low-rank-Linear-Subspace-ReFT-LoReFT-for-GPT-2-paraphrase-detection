...
from models.loreft_strategies import (
    full_finetuning,
    last_two_layers,
    last_four_pos_n_layers,
    last_n_nonpad_all_layers,
    last_s_nonpad_all_layers_no_loop,
    p_first_s_last_nonpad_all_layers_no_loop,
    p_first_s_last_nonpad_all_layers_no_loop_fix,
)
...
class GPT2Model(GPTPreTrainedModel):
  ...
  def __init__(self, config):
    ...
    self.loreft = LoReFT(config.hidden_size, r=32)
    self.prefix_loreft = LoReFT(config.hidden_size, r=32)
    self.suffix_loreft = LoReFT(config.hidden_size, r=32)
  ...
  def encode(self, hidden_states, attention_mask):
    ...
    # ------------------------------------------------------------------ #
    # Choose ONE strategy below. Comment out the rest.                    #
    # ------------------------------------------------------------------ #

    # 1. Full fine-tuning — no LoReFT
    # hidden_states = full_finetuning(
    #   self.gpt_layers, hidden_states, extended_attention_mask
    # )

    # 2. LoReFT on all token positions in the last 2 layers
    # hidden_states = last_two_layers(
    #     self.gpt_layers, hidden_states, extended_attention_mask, self.loreft
    # )

    # 3. LoReFT on last 4 positions by raw index in the last N layers (padding-blind)
    # hidden_states = last_four_pos_n_layers(
    #     self.gpt_layers, hidden_states, extended_attention_mask, self.loreft, num_layers=2
    # )

    # 4. LoReFT on last s non-padding tokens in every layer — per-example loop
    # hidden_states = last_n_nonpad_all_layers(
    #     self.gpt_layers, hidden_states, extended_attention_mask, self.loreft, attention_mask, s=4
    # )

    # 5. LoReFT on last s non-padding tokens in every layer — batched (faster)
    # hidden_states = last_s_nonpad_all_layers_no_loop(
    #     self.gpt_layers, hidden_states, extended_attention_mask, self.loreft, attention_mask, s=4
    # )

    # 6. Prefix + suffix LoReFT in every layer (no overlap protection)
    # hidden_states = p_first_s_last_nonpad_all_layers_no_loop(
    #     self.gpt_layers, hidden_states, extended_attention_mask,
    #     self.prefix_loreft, self.suffix_loreft, attention_mask, p=4, s=6
    # )

    # 7. Prefix + suffix LoReFT in every layer — overlap-safe, paper-compliant
    hidden_states = p_first_s_last_nonpad_all_layers_no_loop_fix(
        self.gpt_layers, hidden_states, extended_attention_mask,
        self.prefix_loreft, self.suffix_loreft, attention_mask, p=4, s=6
    )
